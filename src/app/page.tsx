...
const Home = () => {
  ...
  return (
    <AnimatePresence>
      {loaded && (<motion.div key={"home"} variants={opacity} initial="hidden" animate="visible" exit="exit" transition={{
        duration: 0.4,
        ease: 'easeInOut'
        ...
        <motion.div exit={{
          ...
          transition: {
            ...
            top: { delay: 1, duration: 0.4 },
            height: { delay: 1, duration: 0.2 }
          }
        }} className={css.cardContainer}>
          <AnimatePresence>
            {loaded && (
              <motion.div className={css.containerCover}>
                <motion.div variants={presentCard} initial="hidden" animate="visible" transition={{
                  ...
                  <div className={css.cardContent}>
                    ...
                    <div className={css.social}>
                      <a href="https://instagram.com" onClick={() => { setLoaded(false) }}><motion.div variants={componentopacity} initial="shuru" animate={{ opacity: 0.7, y: 0 }} whileHover={{ opacity: 1, transition: { delay: 0, duration: 0.1, type: spring } }} transition={{ duration: 0.4, delay: 2.9 }} className={css.instagram} /></a>
                      <a href="" onClick={() => { setLoaded(false) }}><motion.div variants={componentopacity} initial="shuru" animate={{ opacity: 0.7, y: 0 }} whileHover={{ opacity: 1, transition: { delay: 0, duration: 0.1 } }} transition={{ duration: 0.4, delay: 3.1 }} className={css.x} /></a>
                      <a href="" onClick={() => { setLoaded(false) }}> <motion.div variants={componentopacity} initial="shuru" animate={{ opacity: 0.7, y: 0 }} whileHover={{ opacity: 1, transition: { delay: 0, duration: 0.1 } }} transition={{ duration: 0.4, delay: 3.3 }} className={css.email} /></a>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0, transition: { delay: 0, duration: 0.4 } }} className={css.exploreContainer}>
                  <div >
                    <Link href={""} onClick={() => { setLoaded(false); }} onAnimationEnd={() => next("/home")}>Explore</Link>
                  </div>
                </motion.div>
              </motion.div>
            )}
